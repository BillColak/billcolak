...
import {useFrame, extend, createPortal, Canvas, useThree} from "@react-three/fiber";
...
function AttractorParticles() {
    ...
    useFrame((state) => {
        ...
        let tempTarget = renderTarget;
        renderTarget = renderTargetClone
        renderTargetClone = tempTarget
    ...
    return (
        <>
            {createPortal(
                <mesh>
                    <testSimulationMaterial ref={simulationMaterialRef} args={[size]} />
                    <bufferGeometry>
                        <bufferAttribute
